...
class TFIDF:
    ...
    def fit(self, raw_documents: List[Union[str, bytes]]):
        ...
        for term in self.vocabulary:
            ...
            self.idf[term] = math.log((doc_count + 1) / (1 + doc_freq)) + 1  # Smoothing added

    def transform(self, raw_documents: List[Union[str, bytes]]) -> sp.csr_matrix:
        ...
        for i, doc in enumerate(raw_documents):
            ...
            for term in term_freq:
                if term in self.vocabulary:
                    ...
                    values.append(self.calculate_tfidf(term, term_freq))

        return sp.csr_matrix((values, (rows, cols)), shape=(len(raw_documents), len(self.vocabulary)))
